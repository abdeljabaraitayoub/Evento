...
export default {
  ...
  methods: {
    ...
    fetchData() {
      api.get('/events/all').then((response) => {
          ...
          <table class="w-full table-auto">
            <thead>
              <tr class="bg-gray-2 text-left dark:bg-meta-4">
                ...
                <th class="py-4 px-4 font-medium text-black dark:text-white">Accept</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in events" :key="item.id">
                ...
                <td class="py-5 px-4">
                  <p class="text-black dark:text-white">{{ item.start_date }}</p>
                </td>
                <td class="py-5 px-4">
                  <p
                  ...
                  >
                    {{ item.is_valid === 1 ? 'Approved' : 'Not approved' }}
                  </p>
                </td>
                <td class="py-5 px-4">
                  <div class="flex items-center justify-center space-x-3.5">
                    <button @click="Validate(item.id)" class="hover:text-primary">
                      <i
                        class="bi"
                        :class="{
                          'bi-check-square-fill': item.is_valid !== 1,
                          ' bi-x-square-fill': item.is_valid === 1
                        }"
                      ></i>
                    </button>
